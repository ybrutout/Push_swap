...
/*   Updated: 2021/08/02 14:23:51 by ybrutout         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../include/push_swap.h"

static void	ft_free_check(t_col **index)
{
	if ((*index)->col_a)
...
static int	ft_error(t_col **index, char *line)
{
	free(line);
	write(1, "Error\n", 6);
	return (-1);
...
static int	get_next_line_mix(t_col **index)
{
	int		ret;
	char	buffer[2];
	char	*line;
...
static int	last_check(t_col **index)
{
	t_num	*col_a;
	t_num	*tmp;
